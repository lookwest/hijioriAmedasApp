...
def analyze_summer_max_temp_by_decade():
    """
    夏の最高気温を10年ごとに分析します。
    """
    try:
        ...
        # 各年代の夏の最高気温を計算
        summer_max_temps = df_summer.groupby('Decade')['最高気温(℃)'].max().reindex(['1980s', '1990s', '2000s', '2010s', '2020s'])

        print("各年代の夏の最高気温:")
        for decade, max_temp in summer_max_temps.items():
            if pd.notna(max_temp):
                print(f"- {decade}: {max_temp:.1f} °C")
            else:
...
if __name__ == '__main__':
    analyze_summer_max_temp_by_decade()
