...
output_image_path = 'img/january_snow_depth_comparison_boxplot.png'

def plot_january_snow_depth_boxplot():
    try:
        # CSVを読み込む
        ...
        # 箱ひげ図の作成
        # 2026年を目立たせるために色を分ける
        colors = ['red' if y == 2026 else 'skyblue' for y in sorted(df_plot['年'].unique())]
        sns.boxplot(x='年', y='最深積雪(cm)', data=df_plot, palette=colors)
        
        # ラベルとタイトル
        plt.xlabel('年')
        plt.ylabel('最深積雪 (cm)')
        plt.title(f'1月の積雪深分布の年次比較 (直近15年間)')
...
if __name__ == '__main__':
    plot_january_snow_depth_boxplot()
