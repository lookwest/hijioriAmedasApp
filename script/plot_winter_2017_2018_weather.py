...
# 対象期間（2017年12月1日～2018年3月31日）のデータを抽出
...
# 445cmを記録した日をハイライト
record_date = pd.to_datetime('2018-02-13')
record_snow_depth = winter_df[winter_df['年月日'] == record_date]['最深積雪(cm)'].iloc[0]
ax1.axvline(record_date, color='magenta', linestyle='--', linewidth=2, label=f'Record Snow (445cm on {record_date.date()})')
ax1.plot(record_date, record_snow_depth, 'm*', markersize=15)

# グラフのフォーマット設定
fig.suptitle('Weather Trend in Winter 2017-2018 (Hijiori)', fontsize=16)
...
# グラフを保存
plt.savefig('winter_2017_2018_weather.png')

print("グラフ 'winter_2017_2018_weather.png' を生成しました。")
