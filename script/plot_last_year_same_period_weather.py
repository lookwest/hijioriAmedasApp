...
# 気温のグラフ (左Y軸)
ax1.plot(df_period['年月日'], df_period['最高気温(℃)'], marker='o', linestyle='-', label='最高気温', color='tab:red')
ax1.plot(df_period['年月日'], df_period['最低気温(℃)'], marker='o', linestyle='-', label='最低気温', color='tab:blue')
ax1.set_xlabel('日付')
...
# 積雪深のグラフ (右Y軸)
ax2 = ax1.twinx()
ax2.bar(df_period['年月日'], df_period['最深積雪(cm)'], label='最深積雪', color='lightblue', alpha=0.7)
...
# タイトル
plt.title(f'1年前の同期間の気象データ ({start_date.strftime("%Y/%m/%d")} - {end_date.strftime("%Y/%m/%d")})')

# レイアウト調整と保存
fig.tight_layout()
output_path = 'img/last_year_same_period_weather.png'
